...
// Producer is an HTTP server made for webthings model
...
func New(ip string, port int, secure bool) *Producer {
	...
	log.Info().Str("url", address).Msg("[producer:NewServer] Server setup")
...
func (p *Producer) Produce(td *thing.Thing) *ExposedThing {
	...
	exposedThing.SetForms(host, p.secure)
...
func (p *Producer) Expose() {
	if p == nil {
		log.Error().Msg("[thingServer:Start] nil server")
	}
