...
func propertyWriteHandler(t *producer.ExposedThing, tdProperty *interaction.Property) func(*fiber.Ctx) error {
	return func(c *fiber.Ctx) error {
		zlog.Trace().Str("uri", c.Path()).Msg("[protocolHttp:propertyWriteHandler] Received Thing property PUT request")
		if tdProperty.ReadOnly {
			...
			if property, ok := t.ExposedProperties[tdProperty.Key]; ok {
				...
				if handler != nil {
					...
					err := handler(t, tdProperty.Key, data, params)
					if err != nil {
					...
					if err := t.EmitPropertyChange(tdProperty.Key, params); err != nil {
